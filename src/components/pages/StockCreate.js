    ...
    <Container className={classes.root}>
      ...
      <div className="box-body" style={{ marginTop: 30 }}>
        <Formik
          validate={(values) => {
            let errors = {};
            if (!values.name) errors.name = "Enter Book name";
            if (!values.category) errors.category = "Enter category";
            if (!values.bookcontent) errors.bookcontent = "Enter Book content";
            return errors;
          }}
          initialValues={{ name: "", category: "", bookcontent: "" }}
          onSubmit={(values, { setSubmitting }) => {
            let formData = new FormData();
            formData.append("name", values.name);
            formData.append("bookcontent", values.bookcontent);
